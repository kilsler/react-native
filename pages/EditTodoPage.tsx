...
export default function EditTodoPage() {
    ...
    useEffect(() => {
        ...
        const fetchTodo = async () => {
            try {
                ...
                if (!res.ok) throw new Error(json.message || "Failed to load todo");

                const data = json.data.data;
                setForm({
                    title: data.title,
                    category_id: data.category_id || null,
                    due_date: data.due_date ? data.due_date.split(" ")[0] : "",
    ...
    const onChangeDate = (event: any, selectedDate?: Date) => {
        setShowDatePicker(Platform.OS === "ios"); // iOS держит открытым
        if (selectedDate) {
            setDueDate(selectedDate);
            setForm({ ...form, due_date: selectedDate.toISOString().split("T")[0] }); // YYYY-MM-DD
        }
    };
